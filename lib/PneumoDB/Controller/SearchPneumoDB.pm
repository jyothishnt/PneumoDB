...
sub getSearchResults() {
  ...
  try {
    $sth = $c->config->{pneumodb_dbh}->prepare($qString) or $resultSet->{err} = "$!";
...
sub createQuery {
      ...
      if($search_data->{search_input}->[$i]->[0]->{eq} !~/groupby/) { # If not Group by then move forward
        ...
        if($search_data->{search_input}->[$i]->[0]->{eq} =~ /REGEXP/) {
          if($search_data->{search_input}->[$i]->[0]->{search_str} eq "") {
            ...
            # Example: WHERE (SC.pss_sanger_id REGEXP "2245STDY5609011|2245STDY5609010")
  ...
  if($download_type eq 'download_selected') {
    ...
    foreach my $row (@{$search_data->{download_selected}}) {
      if($row->{pss_sanger_id}) {
        push @$arr_sanger_id, $row->{pss_sanger_id};
      }
    }
    if($#$arr_sanger_id >= 0) {
      my $str_sanger = '"' . join('","', @$arr_sanger_id) . '"';
      # Directly use Where condition as we dont have any search conditions above
      $search_str_map->{dowload_selected} = " WHERE SC.pss_sanger_id IN ($str_sanger) ";
    }
  }
  ...
  #    $search_str_map->{download_condition} = " AND U.prs_decision <> 0 ";
  # }
  ...
  if(defined $c->user->pnu_institution && $c->user->pnu_institution !~ /Sanger/) {

    my @arr = split(';',$c->user->pnu_institution);
    my $str = join('","', @arr);
    ...
    $search_str_map->{site_specific} .= ' M.pmd_institution IN ("' . $str . '")';
    if($c->user->get('pnu_username') eq "testuser") {
      $search_str_map->{site_specific} .= ' M.pmd_study_name = "Global Strain Bank" ';
    }
  }
...
sub checkPneumoDBionAndExecute() {
  my ($qString, $c) = @_;
  my $sth;
  ...
  try {
    $sth = $c->config->{pneumodb_dbh}->prepare($qString) or die;
    $sth->execute() or die;
  }
  catch {
...
sub getSScapeMapForRepeats {
  ...
  my $q = qq {
    SELECT
      pss_public_name,
      substr(pss_public_name,1,CHAR_LENGTH(pss_public_name)-3) as pss_orig_public_name
    FROM
      pneumodb_sequence_scape
    WHERE
      pss_public_name REGEXP '.*_R[[:digit:]]\$'
    };
  my $sth = $c->config->{pneumodb_dbh}->prepare($q) or die;
  $sth->execute() or die;
  if($sth->rows > 0) {
    while(my $row = $sth->fetchrow_hashref) {
      $map->{'public_name_map'}->{$row->{pss_public_name}} = $row->{pss_orig_public_name};
      push(@{$map->{public_name_orig_arr}}, $row->{pss_orig_public_name});
    }
  }
  return $map;
}
  ...
  if($#$pname_arr >= 0) {
    ...
    my $q = qq {
      ...
      FROM
        pneumodb_metadata
      WHERE
        pmd_public_name IN ("$pname_arr_str")
      };
    my $sth = $c->config->{pneumodb_dbh}->prepare($q) or die $!;
    $sth->execute() or die $!;
    if($sth->rows > 0) {
      while(my $row = $sth->fetchrow_hashref) {
        $map->{$row->{pmd_public_name}} = $row;
      }
    }
  }
  return $map;
