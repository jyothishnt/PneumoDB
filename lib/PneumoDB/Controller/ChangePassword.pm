package PneumoDB::Controller::ChangePassword;
use Moose;
...
PneumoDB::Controller::ChangePassword - Catalyst Controller
...
sub change_password : Path('/changepassword/update') {
  ...
  try {
    ...
    if ($username && $oldpass) {
        ...
        if( $c->authenticate({
              pnu_username => $username,
              pnu_password => md5_hex($oldpass)
            }) ) {

          if($c->user->get("pnu_active")) {
              $q = qq{UPDATE pneumodb_users SET pnu_password = MD5(?) WHERE pnu_username = ? AND pnu_password = MD5(?)};
              my $err_fl = 0;
              try {
                $sth = $c->config->{pneumodb_dbh}->prepare($q) or die $!;
