...
__PACKAGE__->config( 'Plugin::Authentication' =>
                    {
                        ...
                        members => {
                            credential => {
                                class => 'Password',
                                password_field => 'pnu_password',
                                password_type => 'clear'
                            },
                            store => {
                                class => 'DBIx::Class',
                                user_model => 'pneumoDB::PneumodbUser'
                            }
                        }
                    }
    );
...
__PACKAGE__->log( Log::Log4perl::Catalyst->new(
		__PACKAGE__->path_to('pneumodb_log.conf')->stringify
	));
...
__PACKAGE__->config->{pneumodb_dbh} = $dbh;

=encoding utf8

=head1 NAME

PneumoDB - Catalyst based application
...
L<PneumoDB::Controller::Root>, L<Catalyst>
