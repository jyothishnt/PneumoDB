...
var showGrid = function(fl){
  ...
  var dg = $('#dg').edatagrid({
    ...
    onBeforeEdit: function(index, rowData) {
      ...
      if(!rowData.prs_decision) {
        rowData.prs_decision = "";
      }
      if(!rowData.prs_comments) {
        rowData.prs_comments = "";
      }
    ...
    onAfterEdit: function(index, rowData) {
      ...
      if (JSON.stringify(rowData) != before_edit_rowData) {
        var t = {};
        t['pss_sanger_id'] = rowData.pss_sanger_id;
        t['pss_lane_id'] = rowData.pss_lane_id;
...
var updateAllDecision = function(decisionVal) {
  ...
  $.ajax({
    ...
    cache: false,
    data: {
      'select_columns' : ['pss_sanger_id','pss_lane_id','pss_public_name'],
...
var show_GC_images = function() {
  ...
  if(chkdArr) {
    if(chkdArr.length > 0) {
      $.each(chkdArr, function(ind, row) {
        // Push only if lane id is present
        if(row.pss_lane_id && row.pss_lane_id != "") {
          arr.push(row.pss_lane_id);
        }
      });
      if(arr.length <=0) {
